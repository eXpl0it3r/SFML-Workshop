...
bool Map::check_collision(const sf::Vector2f next_position, std::set<int> collision_values) const
{
...
int Map::get_tile_number(const sf::Vector2<std::size_t> tile_position)
{
	return m_tile_map[tile_position.x + tile_position.y * m_map_size.x];
}
...
void Map::generate_map(const sf::Texture& tilesheet)
{
	...
	for (std::size_t x = 0; x < m_map_size.x; ++x)
	{
		...
		{
			const auto tile_number = get_tile_number({ x, y });
